...
def mapx(ax):  # matches bwidow_sw.sv: FILL *11/16, fxs = 480 + rx (X not flipped), FB centre 480
    cx = ax ^ 512; sx = (cx * 11) >> 4; return 480 + (sx - 352)
def mapy(ay):  # matches bwidow_sw.sv orient C: FILL *11/16, fys = 360 - ry (flip Y; FB 720)
    cy = ay ^ 512; sy = (cy * 11) >> 4; return 360 - (sy - 352)
def inb(x, y):
    return 0 <= x < 960 and 0 <= y < 720
...
spur_inside  = [p for p in spurious if 96 <= p[1] <= 623]
spur_outside = len(spurious) - len(spur_inside)
print(f"  spurious INSIDE rows[96,623]:  {len(spur_inside)}  <- MUST be 0 (row-range clear has no gaps)")
print(f"  spurious OUTSIDE rows[96,623]: {spur_outside}  <- marker survivors (HW boot-full-clear handles these)")
